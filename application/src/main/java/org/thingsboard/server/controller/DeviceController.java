...
public class DeviceController extends BaseController {
    ...
    @ApiOperation(value = "Create Or Update Device (saveDevice)", notes = "Create or update the Device. When creating device, platform generates Device Id as "
            ...
            +
            TENANT_OR_CUSTOMER_AUTHORITY_PARAGRAPH) // 为生成API文档 描述租户或客户的权限段落
    @PreAuthorize("hasAnyAuthority('TENANT_ADMIN', 'CUSTOMER_USER')") // 限制谁可以访问此接口
    @RequestMapping(value = "/device", method = RequestMethod.POST) // 指定请求路径和请求方法
    @ResponseBody // 返回响应体
