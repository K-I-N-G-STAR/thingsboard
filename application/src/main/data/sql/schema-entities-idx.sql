...
CREATE INDEX IF NOT EXISTS idx_factory_role_tenant_id
ON factory_role (tenant_id);

CREATE INDEX IF NOT EXISTS idx_factory_role_tenant_enabled
ON factory_role (tenant_id, enabled);


CREATE INDEX IF NOT EXISTS idx_factory_permission_module
ON factory_permission (module);

CREATE INDEX IF NOT EXISTS idx_factory_permission_enabled
ON factory_permission (enabled);


CREATE INDEX IF NOT EXISTS idx_factory_role_permission_role_id
ON factory_role_permission (role_id);

CREATE INDEX IF NOT EXISTS idx_factory_role_permission_tenant_id
ON factory_role_permission (tenant_id);

CREATE INDEX IF NOT EXISTS idx_factory_role_permission_code
ON factory_role_permission (permission_code);

CREATE INDEX IF NOT EXISTS idx_factory_subject_role_subject
ON factory_subject_role (tenant_id, subject_type, subject_id);

CREATE INDEX IF NOT EXISTS idx_factory_subject_role_role_id
ON factory_subject_role (role_id);

CREATE INDEX IF NOT EXISTS idx_factory_subject_role_enabled
ON factory_subject_role (tenant_id, enabled);

CREATE INDEX IF NOT EXISTS idx_factory_role_scope_target
ON factory_role_scope (tenant_id, target_type, target_id);

CREATE UNIQUE INDEX IF NOT EXISTS idx_factory_role_scope_unq
ON factory_role_scope (
    tenant_id,
    target_type,
    target_id,
    scope_type,
    COALESCE(entity_type, ''),
    COALESCE(entity_id, '13814000-1dd2-11b2-8080-808080808080'::uuid)
);

CREATE INDEX IF NOT EXISTS idx_factory_role_scope_entity
ON factory_role_scope (tenant_id, entity_type, entity_id);

CREATE INDEX IF NOT EXISTS idx_factory_role_scope_scope_type
ON factory_role_scope (tenant_id, scope_type);
